 ...
 A Prometheus and Grafana monitoring stack has been added to the lab to provide real-time visibility into system health metrics.
 
 - **Prometheus** scrapes and stores metrics from Node Exporter and Nginx Exporter
 - **Node Exporter** collects OS-level metrics — CPU, memory, disk, and network
 - **Nginx Exporter** collects Nginx stats — active connections and request rate
 - **Grafana** displays all metrics on a web-based dashboard
 
 Configuration files and setup documentation are available in the [`monitoring/`](./monitoring/) folder.
